    ...
    for(int round = 1; round <= 10; round++) {
        ...
        if(round < 10) {
            for(int c = 0; c < 4; c++) {
                ...
                s[4*c]   ^= xtime(a0 ^ a1) ^ h;
                s[4*c+1] ^= xtime(a1 ^ a2) ^ h;
                s[4*c+2] ^= xtime(a2 ^ a3) ^ h;
                s[4*c+3] ^= xtime(a3 ^ a0) ^ h;
            }
        }

        // AddRoundKey
        for(int i = 0; i < 4; i++) {
            uint32_t k = ctx->rk[round * 4 + i];
            s[4*i]   ^= (uint8_t)(k >> 24);
            s[4*i+1] ^= (uint8_t)(k >> 16);
            s[4*i+2] ^= (uint8_t)(k >> 8);
            s[4*i+3] ^= (uint8_t)k;
...
void aes128_cbc_encrypt(
    ...
    for(size_t off = 0; off < len; off += 16) {
        uint8_t block[16];
        for(int i = 0; i < 16; i++) block[i] = in[off + i] ^ iv[i];
...
void aes128_cbc_decrypt(
    ...
    for(size_t off = 0; off < len; off += 16) {
        uint8_t tmp[16];
        aes128_decrypt_block(ctx, in + off, tmp);
